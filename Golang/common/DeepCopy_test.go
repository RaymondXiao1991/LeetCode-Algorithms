...
CopyFloat32s:
	...
	cpyF := DeepCopy(Float32s).([]float32)
...
CopyInterfaces:
	...
	cpyIf := DeepCopy(Interfaces).([]interface{})
...
sliceMap:
	...
	cpyM := DeepCopy(slMap).([]map[int]string)
...
func TestIssue9(t *testing.T) {
	...
	copyA := DeepCopy(testA).(map[string]*int)
	...
	copyC := DeepCopy(testC).(map[*Foo][]string)
	...
	copyD := DeepCopy(testD).(map[Bizz]string)
...
func (i *I) DeepCopyIface() interface{} {
